...
from openvgd.models.mcan.capsatt_sdcam import caps_att
from openvgd.models.mcan.capsatt_visualmap import caps_visual
...
class MCA_ED(nn.Module):
    def __init__(self, __C):
        ...
        self.enc_list = nn.ModuleList([SA(__C) for _ in range(1)])
        self.dec_list_1 = nn.ModuleList([SGA(__C) for _ in range(1)])
        self.dec_list_2 = nn.ModuleList([SGA(__C) for _ in range(1)])
    ...
    def forward(self, x, y, x_mask, y_mask):
        ...
        x_att_mask = att_mask(c_lang, x_mask.squeeze(1).squeeze(1))  # b,100
        mul_x = torch.ones_like(x)  # b,100,512
        mul_x = mul_x.masked_fill(x_att_mask.unsqueeze(-1), 0.3)
        x_att = x * mul_x
        ...
        proj_feat = img_feat_f + lang_feat_f
        proj_feat = self.linear_merge(proj_feat)
        # proj_feat = x + proj_feat.unsqueeze(1)
        proj_feat = x_att + proj_feat.unsqueeze(1)

        proj_feat_2 = img_feat_f_2 + lang_feat_f_2
