...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    loss_kl = eval('torch.nn.' + 'KLDivLoss' + "(reduction='" + "batchmean').cuda()")
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                loss_item_1 = [pred_0, sub_ans_iter]
                loss_item_2 = [pred_1, sub_ans_iter]
                ...
                for item_ix, loss_nonlinear in enumerate(loss_nonlinear_list):
                    if loss_nonlinear in ['flat']:
                        loss_item_1[item_ix] = loss_item_1[item_ix].view(-1)
                        loss_item_2[item_ix] = loss_item_2[item_ix].view(-1)
                    elif loss_nonlinear:
                        loss_item_1[item_ix] = eval('F.' + loss_nonlinear + '(loss_item[item_ix], dim=1)')
                        loss_item_2[item_ix] = eval('F.' + loss_nonlinear + '(loss_item[item_ix], dim=1)')

                # loss = loss_fn(loss_item[0], loss_item[1])
                loss_bce_0 = loss_fn(loss_item_1[0], loss_item_1[1])
                loss_bce_1 = loss_fn(loss_item_2[0], loss_item_2[1])
                kl_loss_01 = loss_kl(F.log_softmax(loss_item_1[0], dim=1), F.softmax(Variable(pred_1), dim=1))
                kl_loss_10 = loss_kl(F.log_softmax(loss_item_2[0], dim=1), F.softmax(Variable(pred_0), dim=1))
